...
ALTER TABLE ONLY "public"."claims"
    ADD CONSTRAINT "claims_item_claimer_unique" UNIQUE ("item_id", "claimer_id");
...
CREATE UNIQUE INDEX "idx_claims_item_claimer_unique" ON "public"."claims" USING "btree" ("item_id", "claimer_id");



COMMENT ON INDEX "public"."idx_claims_item_claimer_unique" IS 'Unique constraint to ensure idempotency of claim operations and support ON CONFLICT.';
...
CREATE INDEX "idx_lists_id_event_created" ON "public"."lists" USING "btree" ("id", "event_id", "created_by");
...
CREATE POLICY "Original claimers can update split requests" ON "public"."claim_split_requests" FOR UPDATE TO "authenticated" USING (("auth"."uid"() = "original_claimer_id")) WITH CHECK (("auth"."uid"() = "original_claimer_id"));



CREATE POLICY "Requesters can delete their pending requests" ON "public"."claim_split_requests" FOR DELETE TO "authenticated" USING ((("auth"."uid"() = "requester_id") AND ("status" = 'pending'::"text")));



CREATE POLICY "Users can create split requests" ON "public"."claim_split_requests" FOR INSERT TO "authenticated" WITH CHECK (("auth"."uid"() = "requester_id"));
...
CREATE POLICY "Users can view their split requests" ON "public"."claim_split_requests" FOR SELECT TO "authenticated" USING ((("auth"."uid"() = "requester_id") OR ("auth"."uid"() = "original_claimer_id")));
...
CREATE POLICY "event_member_stats_select" ON "public"."event_member_stats" FOR SELECT USING (("user_id" = "auth"."uid"()));



COMMENT ON POLICY "event_member_stats_select" ON "public"."event_member_stats" IS 'Users can only view their own statistics';
...
CREATE POLICY "items_select_with_receiver_assignment" ON "public"."items" FOR SELECT USING ((EXISTS ( SELECT 1
  ...
  WHERE (("l"."id" = "items"."list_id") AND ("em"."user_id" = "auth"."uid"()) AND "public"."can_view_list"("l"."id", "auth"."uid"()) AND ((("l"."random_assignment_enabled" = true) AND ("l"."random_receiver_assignment_enabled" = true)) OR ("l"."created_by" = "auth"."uid"()) OR ("em"."role" = 'admin'::"public"."member_role") OR (EXISTS ( SELECT 1
           FROM "public"."events" "e"
          WHERE (("e"."id" = "l"."event_id") AND ("e"."owner_id" = "auth"."uid"())))) OR (("l"."random_assignment_enabled" = true) AND (COALESCE("l"."random_receiver_assignment_enabled", false) = false) AND (EXISTS ( SELECT 1
           FROM "public"."claims" "c"
...
COMMENT ON POLICY "items_select_with_receiver_assignment" ON "public"."items" IS 'Users can see items in lists they can view. For combined random assignment (giver+receiver), all members see all items for collaborative planning. For single random assignment modes, visibility is restricted. List creators/admins/owners always see all items.';
...
CREATE POLICY "list_exclusions_delete" ON "public"."list_exclusions" FOR DELETE TO "authenticated" USING (((EXISTS ( SELECT 1
   FROM (("public"."lists" "l"
     JOIN "public"."events" "e" ON (("e"."id" = "l"."event_id")))
     JOIN "public"."event_members" "em" ON (("em"."event_id" = "e"."id")))
  WHERE (("l"."id" = "list_exclusions"."list_id") AND ("em"."user_id" = "auth"."uid"()) AND (("l"."created_by" = "auth"."uid"()) OR ("em"."role" = 'admin'::"public"."member_role") OR ("e"."owner_id" = "auth"."uid"()))))) OR ("user_id" = "auth"."uid"())));
