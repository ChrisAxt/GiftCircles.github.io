-- Migration: Revert events_for_current_user back to original version
-- Date: 2025-01-17
-- Description: Revert migration 040 - restore the original claimed_count logic that filters by l.created_by
...
AS $function$
  ...
  claims as (
    -- Restored: Filter by l.created_by to show claims on lists created by current user
...
'Returns all events for the current user with counts. claimed_count shows claims on lists created by the current user.';
