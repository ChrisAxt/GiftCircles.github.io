-- Migration: Update create_list_with_people to support for_everyone parameter
-- Date: 2025-01-17
-- Description: Add p_for_everyone parameter to create_list_with_people function to match migration 039

BEGIN;

-- Drop and recreate function with new parameter
DROP FUNCTION IF EXISTS public.create_list_with_people(uuid, text, list_visibility, uuid[], uuid[], uuid[], text, boolean, text, boolean);
...
BEGIN
  ...
  -- Create list with all fields including random assignment settings and for_everyone
  INSERT INTO lists (
    event_id,
    name,
    created_by,
  ...
  IF array_length(p_recipients, 1) IS NOT NULL THEN
    INSERT INTO list_recipients (list_id, user_id, can_view)
  ...
  IF COALESCE(p_visibility, 'event') = 'selected'
    ...
    INSERT INTO list_viewers (list_id, user_id)
    SELECT v_list_id, v
